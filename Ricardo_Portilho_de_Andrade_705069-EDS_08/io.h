...
// Meus metodos 
/*
Separador - separar imformacoes para melorar leitura 
@param - void 
*/
void Separador (void)
{
 int x  = 200; // quantidade de repeticao
 IO_printf("\n"); // evitar erro
 while (x > 0) // repeticao
 {
  IO_printf("_"); // mostrar separador
  x = x - 1; // controlar repeticao
 } 
 IO_printf("\n"); // evitar erro
}
/*
MIntArray - mostrar valores de um arranjo com valores inteiros 
@param n - Quantidade de dados do arranjo
@param array [] - arranjo que sera' lido
*/
void MIntArray ( int n ,int array [ ] )
{
 // definir dados 
 int y = 0; // controle
 Separador ();// separar informacoes
 for( y = 0; y < n; y = y + 1 )// repetir
 {
  IO_printf("\n%2d : %d", y, array[ y ]); // mostrar
 }
 Separador ();// separar imformacoes
}
